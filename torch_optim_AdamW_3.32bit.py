...
class PQ:
    ...
    def get_theta(points):
        cos = torch.acos((2 - (points[..., 0] ** 2 + points[..., 1] ** 2)) / 2)
        pq = (torch.pi - cos) / 2
        pq = torch.atan2(points[..., 1], points[..., 0]) - pq
        pq[pq < 0] += 2 * torch.pi
        dd = torch.pi + pq - cos
        ...
        dd = dd // 10
        pq = pq // 10
        return (dd * 10 + pq).to(PQ.theta_type)        # 0~99

    @staticmethod
    def encode(tensor):
        shape = tensor.shape
        tensor = tensor.reshape(-1, 32)
        Xs = tensor[..., :: 2]
        Ys = tensor[..., 1 :: 2]
        rX, _ = torch.max(tensor.abs(), dim = -1, keepdim= True)
        rX[rX == 0.] = 1e-40
        Xs = Xs / rX
        Ys = Ys / rX
        weight_T = torch.stack([Xs, Ys], dim = -1)
        X = PQ.get_theta((weight_T)).reshape(shape[:-1] + (shape[-1] // 2,))
        X.rX = rX
        X.ori_shape = shape
        return X
    ...
    @staticmethod
    def decode(tensor):
        X = torch.stack([PQ.tx(tensor.reshape(-1, 16)) * tensor.rX, PQ.ty(tensor.reshape(-1, 16)) * tensor.rX], dim = -1)
        X = X.reshape(*tensor.ori_shape)
        return X
    
    @staticmethod
    def assign(para, tensor):
        para.copy_(tensor)
        para.rX = tensor.rX
        para.ori_shape = tensor.ori_shape
        ...
        for p in group["params"]:
            ...
            if len(state) == 0:
                ...
                state["exp_avg"] = torch.zeros_like(
                    p[...,::2], memory_format=torch.preserve_format
                ).to(dtype=PQ.theta_type) if p.shape[-1] % 32 == 0 else torch.zeros_like(
    ...
    for i, param in enumerate(params):
        ...
        if lens % 32 == 0:
            if step_t.item() == 1:
                ...
                exp_avg_f = PQ.decode(exp_avg)
                exp_avg_sq_f = torch.abs(PQ.decode(exp_avg_sq))
        else:
            exp_avg_f = exp_avg
            exp_avg_sq_f = exp_avg_sq

        # assert not torch.isnan(exp_avg_f).any()
        # assert not torch.isnan(exp_avg_sq_f).any()

        exp_avg_f.lerp_(grad, 1 - beta1)
        exp_avg_sq_f.mul_(beta2).addcmul_(grad, grad, value=1 - beta2)

        if not exp_avg_f.shape == grad.shape:
            print('err')
        if capturable or differentiable:
            ...
            if amsgrad:
                ...
                denom = (exp_avg_sq_f.sqrt() / bias_correction2_sqrt).add_(eps)

            param.addcdiv_(exp_avg_f, denom, value=-step_size)
        ...
        if lens % 32 == 0:
            ds = PQ.encode(exp_avg_f)
            PQ.assign(exp_avg, ds)
            ds = PQ.encode(torch.abs(exp_avg_sq_f))
            PQ.assign(exp_avg_sq, ds)
